...
async def export_network_csv_to_file(filepath: str = "network.csv") -> str:
    ...
    # Optionally run local curl to re-download (local machine context)
    curl_url = "http://localhost:8000/file-csv"  # Or your local api URL
    # Save to different file name to avoid confusion or overwrite:
    curl_output = os.path.join(os.path.dirname(filepath), "linkedin_network_downloaded.csv")
    try:
        subprocess.run([
        ...
        return f"Export done, but curl failed: {e}"
    
    return f"CSV exported to {filepath}. CURL downloaded to {curl_output}."
